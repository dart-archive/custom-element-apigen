...
hyd.Analyzer.analyze(filePath)
    .then(function(results) {
      console.log(JSON.stringify({
        ...
        //behaviors: buildBehaviors(results)
      }));
    });
...
function getProperties(element) {
  ...
  for (var i = 0; i < element.properties.length; i++) {
    ...
    properties.push({
      ...
      name: property.name,
      type: getFieldType(property, properties),
...
function getFieldType(property, properties) {
  if (!property.function) return property.type;
  if (isGetter(property)) return property.return ? property.return.type : null;
  if (isSetter(property)) {
    for (var i = 0; i < properties.length; i++) {
      var prop = properties[i];
      if (prop.name == property.name && isGetter(prop)) {
        return prop.return ? prop.return.type : null;
      }
    }
  }
}
